...
monk.dat$Difference <- monk.dat$After - monk.dat$Before
```

.pull-left[
```{r}
xbar <- mean(monk.dat$Difference)
n <- length(monk.dat$Difference)
se <- sd(monk.dat$Difference)/sqrt(n)
...
n <- length(monk.dat$Difference)
se <- sd(monk.dat$Difference)/sqrt(n)
t <- (mean(monk.dat$Difference) - 0)/se
t
2*pt(t, df=n-1)
```
]
...
```{r}
  t.test(x=monk.dat$Difference)
  t.test(x=monk.dat$Before, y=monk.dat$After, paired=TRUE)
```
]
...
- We looked at one- and two-sample t-tests
