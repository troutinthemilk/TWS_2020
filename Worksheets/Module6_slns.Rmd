...
dace.dat <- dace.dat[,-1]#remove stream name from the dataset. this will simplify further analyses
...
mod1.xv <- train(TotalSleep ~ LifeSpan + Exposure, data=sleep.dat, method = "lm", na.action=na.omit, trControl=sleep.control)
...
```{r}
read.csv(file="../Data/MammalSleep.csv")
mod1 <- lm(TotalSleep ~ LifeSpan + Exposure, data=mammal.dat)
mod2 <- lm(TotalSleep ~ Danger + Predation, data=mammal.dat) 
mod3 <- lm(TotalSleep ~ Exposure, data=mammal.dat)
library(MuMIn)
